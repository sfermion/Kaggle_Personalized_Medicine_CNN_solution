...
train_txt = pd.read_csv('Data/training_text', sep="\|\|", engine='python', header=None, skiprows=1, names=["ID","Text"])
test_var = pd.read_csv('Data/test_variants')
test_txt = pd.read_csv('Data/test_text', sep="\|\|", engine='python', header=None, skiprows=1, names=["ID","Text"])

train = pd.merge(train_var, train_txt, how='left', on='ID').fillna('')
y = train['Class'].values
train = train.drop(['Class'], axis=1)

test = pd.merge(test_var, test_txt, how='left', on='ID').fillna('')
pid = test['ID'].values
...
Feature Engineering:
	1. Variation code: 
		GPG (3 to 6 digits) or		
		word ("del","deletion","fusion","Truncating" "Amplification")
			"G-G fusion" "G_G"
	we can extract the two amino acise fist letterse and the position of the gene from LNL

3rd parties:
	http://mygene.info/v3/query?q=<gene name> : gives us the name of the gene
	Word2Vec
	Doc2Vec
	tmvar
	taggerone
